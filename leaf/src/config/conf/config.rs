...
pub struct Config {
    ...
    pub rule: Vec<Rule>,
    pub host: HashMap<String, Vec<String>>,
}
...
pub fn to_internal(conf: &mut Config) -> Result<internal::Config> {
    ...
    if let Some(ext_loglevel) = &conf.general.loglevel {
    ...
    if let Some(ext_logoutput) = &conf.general.logoutput {
        match ext_logoutput.as_str() {
            "console" => log.output = protobuf::EnumOrUnknown::new(internal::log::Output::CONSOLE),
            _ => {
    ...
    if conf.general.http_interface.is_some() && conf.general.http_port.is_some() {
        let mut inbound = internal::Inbound::new();
        inbound.protocol = "http".to_string();
        inbound.tag = "http".to_string();
        inbound.address = conf.general.http_interface.as_ref().unwrap().to_string();
        inbound.port = conf.general.http_port.unwrap() as u32;
        inbounds.push(inbound);
    }
    if conf.general.socks_interface.is_some() && conf.general.socks_port.is_some() {
        let mut inbound = internal::Inbound::new();
        inbound.protocol = "socks".to_string();
        inbound.tag = "socks".to_string();
        inbound.address = conf.general.socks_interface.as_ref().unwrap().to_string();
        inbound.port = conf.general.socks_port.unwrap() as u32;
        inbounds.push(inbound);
    }

    if conf.general.tun_fd.is_some()
        || conf.general.tun_auto.is_some()
        || conf.general.tun.is_some()
    {
        let mut inbound = internal::Inbound::new();
        ...
        if let Some(ext_always_real_ip) = &conf.general.always_real_ip {
        ...
        if let Some(ext_always_fake_ip) = &conf.general.always_fake_ip {
        ...
        if conf.general.tun_fd.is_some() {
            settings.fd = conf.general.tun_fd.unwrap();
        } else if conf.general.tun_auto.is_some() && conf.general.tun_auto.unwrap() {
            settings.auto = true;
            settings.fd = -1; // disable fd option
        } else {
            let ext_tun = conf.general.tun.as_ref().unwrap();
    ...
    for ext_proxy_group in &conf.proxy_group {
        let mut outbound = internal::Outbound::new();
    ...
    let mut rules = Vec::new();
    for ext_rule in conf.rule.iter_mut() {
    ...
    if let Some(ext_domain_resolve) = conf.general.routing_domain_resolve {
    ...
    if let Some(ext_dns_servers) = &conf.general.dns_server {
    ...
    for (name, static_ips) in conf.host.iter() {
